...
export function GalleryPreview() {
    ...
    // We map colors to the translated categories based on index
    const colors = ["bg-neutral-800", "bg-neutral-700", "bg-neutral-600"];

    const categories = t.gallery.categories.map((cat, idx) => ({
        ...cat,
        color: colors[idx]
    }));
        ...
        <section className="w-full py-24 px-6 bg-paper-white relative">
            <div className="max-w-7xl mx-auto">
                <div className="flex justify-between items-end mb-12">
                    <div>
                        <h2 className="text-4xl text-ink-black font-calligraphy mb-2">{t.gallery.title}</h2>
                    ...
                    <button className="group flex items-center gap-2 text-ink-black font-serif hover:text-seal-red transition-colors">
                        {t.gallery.viewAll} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </button>
                </div>
